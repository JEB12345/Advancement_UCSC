...
\section{/SB{} v1 Design Evaluation and Future Work}
...
\textcolor{red}{
 - Friction in bowden cable 
 - Spindle diameter to large for cable tensions 
 - Spring cable exit/entrance to MTR was poorly designed 
 - Large spring buckling under large deformations 
 - Heavier than original design specs
 - Torque and Force sensors never worked
}
...
\begin{figure}[thpb]
      \centering
      \includegraphics[width=0.6\columnwidth]{tex/img/cable_pulley_bearing_labelled_fixedfonts}
      \caption{Temp. figure. Explain how the pictures show hysteresis due to cable friction.}
...
These values were obtained through evaluating the tension forces derived from Atil et al's control work within the NASA Tensegrity Robotics Toolkit~\cite{iscen2014flop}.
...
Each motor has a Maxon gearbox with a maximum output torque of \(3Nm\) originally designed to be coupled to a spool with a diameter of \(30mm\).
Using the spool's radius and the robot's operating tensions listed above, torques are calculated in equations~\ref{tau_nom} and~\ref{tau_max}.

\begin{align}
\tau_{nominal} = 240N \times 0.015m = 3.6Nm \label{tau_nom} \\
\tau_{maximum} = 400N \times 0.015m = 6.0Nm \label{tau_max}
\end{align}

These values are well above the continuous torque that the motor's gearbox is rated to handle.
To keep from breaking all the motors when operating the robot, smaller diameter spools were designed. 
The trade off is that the cable change in length over time is decreased, forcing the robot to move slower.
...
\begin{align}
\tau_{nominal, new spool} = 240N \times 0.009m = 2.7Nm \label{new_nom} \\
\tau_{maximum, new spool} = 400N \times 0.009m = 3.6Nm \label{new_max}
\end{align}

This reduced spool is a compromise between lowering torque on the motor and actuation velocity.
The torques induced onto the gearbox could still go over the rated value, but keeps the reduction in speed above \(50\%\).
...
\textcolor{red}{
 ...
 - Sensor Board desgin
   > Communication lines were not correctly designed (impedance issues)
