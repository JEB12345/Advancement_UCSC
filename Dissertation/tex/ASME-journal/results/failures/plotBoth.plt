...
set ylabel "Distance Rolled (m)"
#set xrange auto
set yrange [-5:35]
set y2range [-0.01:0.05]
set size 1,0.5
set y2tics
set ytics nomirror
set output "learningVsFailures.eps"

plot\
 "failures.csv" using 1:($4)/10 title "Best Policy"  w l axes x1y1,\
 "failures.csv" using 1:($2) title "Average Failures"  w l axes x1y2,\
