...
set key right top
set xlabel "Motor Speed (m/s)"
set ylabel "Distance moved in 60 seconds (m)"
#set yrange [0:4]

set output "complexity.eps"
set xlabel "Number of points used to generate signal"

plot\
 "summary.csv" using 1:($10)/10:xtic(1) notitle  w boxes,\
 "" using 1:($10)/10:($11)/10 notitle w yerror ls 1
