...
\textbf{Product} & MX-106    & K-58   & \begin{tabular}[c]{@{}c@{}}custom motor \\ control boards\end{tabular} & X8-3          \\ \hline
\end{tabular}%
}
\end{table}
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{fig:sbv2_far}
\end{subfigure}
\caption{Images of initial \SB{} v2 end cap designs. (a) Close up of initial design for \SB{} v2. An omni directional cable guide can be seen which eliminates cable friction and is based on a cable routing design from Friesen \etal~\cite{}. (b) Shows the full cable routing where the cable is run over a distal pulley to increase cable length for a better effective spring constant induced by the cable stretch.}
\label{fig:sbv2}
\end{figure}
...
\textbf{\begin{tabular}[c]{@{}c@{}}\SB{} \\ v2\end{tabular}} & 24                                                                                             & \(\SI{1.8}{\meter} \text{/} \SI{30}{\kilo\gram}\)                      & \begin{tabular}[c]{@{}l@{}}Position\\ Velocity\\ Torque\end{tabular} & \begin{tabular}[c]{@{}l@{}}Motor Encoders\\ Motor Torque Sensor\\ Accelerometer, Gyro\\ Ranging / localization Sensors\end{tabular} & \(\SI{1.5}{\meter}\)                                                                                             \\ \hline
\end{tabular}
}
...
The redesign of \SB{} is still in very early stages of developement, however table~\ref{sb_versions_compare} shows some of the differences in base physical properties and features between the two versions.

\subsection{Extending Controls for a Fully Actuated \SB{} v2}
\label{sec:future_controls}

Since \SB{} v2 will have actuators on all cables, the control techniques developed and implemented on the existing \SB{} should still be valid.
